  ...
  return (
    ...
    <Suspense fallback={<BarLoader width={"100%"} color="hsl(178.5deg 52.63% 29.8%) text-yellow" />}>
      <Tabs value={tabValue} className="w-full mt-5">
        <TabsList className="grid grid-cols-5 w-full bg-white text-[hsl(178.5,52.96%,38.6%)] mb-4 pb-4"> {/* Increased padding (py-4) for height */}
          <TabsTrigger
            value="dashboard"
            asChild
            className={`${getTabClass("dashboard")} flex items-center justify-center border-r  last:border-r-0 rounded-none`}
            data-state={tabValue === "dashboard"}>
            <Link href={`/project/${projectId}/dashboard`} className="w-full text-center">
              Dashboard
            </Link>
          </TabsTrigger>
          <TabsTrigger
            value="backlog"
            asChild
            className={`${getTabClass("backlog")} flex items-center justify-center border-r  last:border-r-0 rounded-none`}
          ...
          <TabsTrigger
            value="sprints"
            asChild
            className={`${getTabClass("sprints")} flex items-center justify-center border-r  last:border-r-0 rounded-none`}
      ...
      {/* Remove the default gray background by updating this div's class */}
      <div className="mt-6 p-6 rounded-lg bg-transparent">
        {children}
      </div>
    </Suspense>
  </div>
